...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    h1: ({ children }) => (
      <h1 className="[font-size:_clamp(40px,6vw,60px)] text-red py-24">
        {children}
      </h1>
    ),
    h2: ({ children }) => (
    ...
    p: ({ children }) => (
      <h4 className="[font-size:_clamp(20px,2vw,15px)] mb-4 font-sans">
        {children}
      </h4>
    ),
    h6: ({ children }) => (
      <p className="[font-size:_clamp(20px,4vw,25px)] opacity-50 mb-2">
        {children}
      </p>
    ),
    a: ({ href, children }) => (
